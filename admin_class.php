...
Class Action {
	...
	function get_parcel_heistory(){
		...
		if($parcel->num_rows <=0){
			...
			$data[] = array('status'=>'Artículo aceptado por paquetería','date_created'=>date("M d, Y h:i A",strtotime($parcel['date_created'])));
			$history = $this->db->query("SELECT * FROM parcel_tracks where parcel_id = {$parcel['id']}");
			$status_arr = array("Artículo aceptado por paquetería","Recogida","Enviada","En-Transito","Llegó a destino","En salida para entrega","Listo para recoger","Entregada","Recogida","Intento de entrega fallida");
