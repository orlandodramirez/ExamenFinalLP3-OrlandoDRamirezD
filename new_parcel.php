	...
	<div class="card card-outline card-primary">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input type="checkbox" name="type" id="dtype" <?php echo isset($type) && $type == 1 ? 'checked' : '' ?> data-bootstrap-switch data-toggle="toggle" data-on="Delivery" data-off="Pickup" class="switch-toggle status_chk" data-size="xs" data-offstyle="info" data-width="5rem" value="1">
              <small>Pickup = Entregar a la dirección del destinatario</small>
              <small>, Delivery = Recogida en sucursal más cercana</small>
            </div>
          </div>
        ...
        <table class="table table-bordered" id="parcel-items">
          ...
          <tfoot>
            ...
            <th class="text-right" id="tAmount">0</th>
            <th></th>
          </tfoot>
              <?php endif; ?>
        </table>
        ...
        <div class="row">
          <div class="col-md-12 d-flex justify-content-end">
            <button  class="btn btn-sm btn-primary bg-gradient-primary" type="button" id="new_parcel"><i class="fa fa-item"></i> Agregar artículo</button>
          </div>
        </div>
              <?php endif; ?>
      </form>
...
<script>
  ...
  $('#new_parcel').click(function(){
    ...
    if($('#parcel-items tbody tr').length <= 0){
      alert_toast("Agregue al menos 1 información del paquete.","error")
      end_load()
      return false;
    }
		$.ajax({
        ...
        if(resp == 1){
            alert_toast('Datos guardados exitosamente',"éxito");
            setTimeout(function(){
  ...
  function calc() {
    ...
    if ($('#tAmount').length > 0) {
        $('#tAmount').text(parseInt(total).toLocaleString('en-US') + '.gs');
    }
}
</script>